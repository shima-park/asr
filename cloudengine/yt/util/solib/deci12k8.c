  ...
  int band)           /* input:  0=0..6.4k, 1=6.4..10.8k */
{
  ...
  interpol(signal+ncoef, sig12k8, L_FRAME_PLUS, filter, ncoef, 
           fac_up, fac_down, gain);
  for (i=0; i<(2*ncoef); i++) {
    mem[i] = signal[i+lg];
  }
  return;
  ...
  )			  
{
  ...
  ncoef = NCOEF_12k8;
  if	(lg == L_SUBFR16k)	L_frame = L_SUBFR;
  else						L_frame = L_FRAME_PLUS;
  for (i=0; i<(2*ncoef); i++) {
    signal[i] = mem[i];
  }
  for (i=0; i<L_frame; i++) {
  ...
  if (add==0) {
    interpol(signal+ncoef, sig_fs, lg, filter, ncoef, 
             fac_up, fac_down, gain);
  } else {
    interpol_add(signal+ncoef, sig_fs, lg, filter, ncoef, 
                 fac_up, fac_down, gain);
  }
  for (i=0; i<(2*ncoef); i++) {
